...
/**
 * @description 视图类，负责将模板数据渲染成文本或图片。
 * @class View
 */
export class View {
  /**
  ...
  /**
   * @description 将模板列表格式化为分页文本。
   * @param {MemeInfo[]} list - 完整的模板信息列表
  ...
  /**
   * @description 将单个模板的详细信息格式化为文本。
   * @param {MemeInfo} item - 模板信息对象
  ...
  /**
   * @description 使用 Puppeteer 将模板列表渲染成图片。
   * @param {MemeInfo[]} list - 模板信息列表
   * @returns {Promise<Buffer>} - 包含渲染结果的图片 Buffer
   */
  ...
  /**
   * @description 使用 Puppeteer 将模板详情渲染成图片。
   * @param {MemeInfo} item - 模板信息对象
   * @param {string} [previewData] - 可选的预览图 base64 数据
   * @returns {Promise<Buffer>} - 包含渲染结果的图片 Buffer
   */
